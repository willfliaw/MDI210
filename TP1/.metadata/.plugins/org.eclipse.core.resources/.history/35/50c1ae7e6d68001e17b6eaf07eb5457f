...
public class Dictionary extends Observable {
  ...
  // Slack variables have numbers between 1 + nbNonBasisVar + 1 and nbBasisVar +
  // nbNonBasisVar
  ...
  private int[] arrayBasicVar; // contains from index 1 the numbers of the basic variables, in the order of D
                               // (the index 0 is not used)
  int[] arrayNonBasicVar; // contains from index 1 the numbers of the non-basic variables, in the order of
                          // D (the box of index 0 is not used)

  private double[][] D;
  // For the modeled dictionary:
  // D[0][0] contains the constant of the function z
  // for j which varies from 1 to nbNonBasicVar;, D[0][j] contains the coefficient
  // in the function z of the non-basic variable of index j
  // for i which varies from 1 to nbBasisVar, D[i][0] contains the constant term
  // of the expression of the basis variable of index i
  // for i which varies from 1 to nbBasisVar and for j which varies from 1 to
  // nbNonBasicVar, D[i][j] contains, for the basic variable
   ...
   * Knowing the number of rows and columns, the allocation of the arrays needed
   * for
   * coding is done by this constructor.
   *
   * @param nbBasicVar    number of constraints
   * @param nbNonBasicVar number of decision variables (main variables)
   * @param method        specifies the method to adopt for the choice of the
   *                      entering
   *                      variable
   * @param bland         specifies if one uses the rule of Bland
   */
  ...
  /**
   * TO IMPLEMENT; we look for the index in arrayNonBasicVar of the "first"
   * candidate to enter in basis
   * by considering the variables in the order they occupy in arrayNonBasicVar.
   * For example, suppose that D[0][1] <= 0 and that D[0][2]> 0. The method then
   * returns the value 2.
   * If arrayNonBasicVar[2] for example contains the value 3, it means that the
   * variable x3 is chosen as
   * entering variable but this does not intervene in the programming.
   * For this search, we go through D[0][j] for j which varies from 1 to
   * nbNonBasicVar and we keep the index
   * corresponding to the first strictly positive coefficient encountered if there
   * is such an index.
   * The numbers of the variables do not matter.
   *
   * @return If there is an entering variable, returns the smallest index of an
   *         entering variable, that is,
   *         the smallest positive index of a box of the array D[0][...]
   *         containing a strictly positive value.
   *         otherwise, returns 0.
   */
   ...
   *
   * @param jE the index in arrayNonBasicVar of the entering variable, i.e. the
   *           index of the column
   *           of D that will be used in this method.
   *           In this method, we must use D[i][0] and D[i][jE] for the index i
   *           which varies from 1 to nbBasicVar.
   * @return If the dictionary shows that the problem is not bounded, returns 0.
   *         Otherwise, returns the index in arrayBasicVar of a leaving variable,
   *         i.e. the index of the row of
  ...
  public int searchIndexLeavingVariable(int jE) {
    ...
    for (int i = 1; i <= nbBasicVar; i++)
      if (D[i][jE] < 0 && (unbounded || -D[i][0] / D[i][jE] < max)) {
        max = -D[i][0] / D[i][jE];
        index = i;
        unbounded = false;
      }
  ...
  /**
   * TO IMPLEMENT; modifies the current dictionary (referenced by this) by
   * pivoting according to
   * the indicated parameters .
   *
   * @param iS the index in arrayBasicVar of the leaving variable (i.e. index in D
   *           of the line from
   *           from which we pivot)
   * @paramJE the index in arrayNonBasicVar of the entering variable (i.e. index
   *          in D of the column
   *          from which we pivot)
   */
  /*
   * * Modify the dictionary:
   * the entering variable enters in the basis and the leaving variable leaves the
   * basis.
   ...
   * is invoked with the parameter jE which is equal to 1 and the parameter iS
   * which is equal to 2.
  ...
  public void pivote(int iS, int jE) {
    ...
    arrayBasicVar[iS] = EnteringVar;
    arrayNonBasicVar[jE] = LeavingVar;
  ...
  /**
   * TO IMPLEMENT; look for an entering variable of greatest coefficient in the
   * objective function of the current dictionary.
   * The numbers of the variables do not matter. Only D[0][j] for j which varies
   * from 1 to nbNonBasisVar is concerned.
   *
   * @return If there is an entering variable, returns the index in
   *         arrayNonBasicVar of the entering variable of
   *         greatest coefficient in the objective of the current dictionary,
   *         i.e. the index >= 1 in D [0][...] of the greatest value box.
   *         <br>
   *         otherwise, returns 0c
   */
  ...
  /**
   * To implement; looks for an entering variable that maximizes the increase of
   ...
   *         <br>
   *         otherwise if there is an entering variable that gives an infinite
   *         increase (problem not bounded),
   *         returns the value -1;
   *         <br>
   *         otherwise returns the index in arrayNonBasicVar of the entering
   *         variable that maximizes the increase
   *         of the objective function in the next dictionary.
   */
  public int searchEnteringAdvantageousVariableIndex() {
    double max = 0, min = (int) Float.POSITIVE_INFINITY;
    int index = 0;
    boolean unbounded;
    ...
    for (int j = 1; j <= nbNonBasicVar; j++) {
      ...
      for (int i = 0; i <= nbBasicVar; i++)
        if (D[i][j] < 0 && (unbounded || -D[i][0] / D[i][j] < min)) {
          min = -D[i][0] / D[i][j];
          unbounded = false;
        }
  ...
  /**
   * TO IMPLEMENT: look for the index of the entering variable of smallest number
   * (the variable x_i
   * has number i). Serves when using Bland's rule.
   * For example, if arrayNonBasicVar contains from index 1 the numbers 6, 7, 2,
   * 3, 5 and if the variables
   * candidate to enter are the variables of indices 2 and 4 (arrayNonBasicVar[2]
   * is 7, arrayNonBasicVar[4] is 3),
   * the returned value is 4.
   *
   * @return If there is an entering variable, returns the index in
   *         arrayNonBasicVar of the entering variable of smallest number.
   *         <br>
   *         Otherwise, returns 0.
   */
  public int searchIndexEnteringVariableSmallestNumber() {
    ...
    for (int j = 1; j < nbNonBasicVar; j++)
      if (D[0][j] > 0 && (notFound || arrayNonBasicVar[j] < min)) {
        min = arrayNonBasicVar[j];
        index = j;
        notFound = false;
      }
  ...
  /**
   * TO IMPLEMENT; an entering variable being chosen, look for the index of the
   * leaving variable of smallest number
   * (the variable x_i has number i). Serves when using Bland's rule.
   * For example, if arrayBasicVar contains from index 1 the numbers 4, 5, 1, 2, 7
   * and if the variables candidate to leave
   * are the variables of indices 2 and 4 (arrayBasicVar[2] is 5, arrayBasicVar[4]
   * is 2), the returned value is 4.
   *
   * @paramJE the index of the entering variable.
   * @return If the dictionary shows that the problem is not bounded, returns -1.
   *         <br>
   *         else, returns the index of the leaving variable.
   */
  public int searchIndexLeavingVariableSmallestNumber(int jE) {
    int index = 0, min = (int) Float.POSITIVE_INFINITY;
    boolean unbounded = true;

    for (int i = 1; i <= nbBasicVar; i++)
      if (D[i][jE] < 0 && (unbounded || arrayBasicVar[i] < min)) {
        min = arrayBasicVar[i];
        index = i;
        unbounded = false;
      }
   ...
   * <br>
   * If step has shown that the problem is not bounded, set the terminal attribute
   * to false
  ...
  /**
   * Computes a first dictionary, not feasible, for the first phase.
   * <br>
   * The introduced variable is named x0 (in other words it has the number 0) and
  ...
  public Dictionary firstAuxiliaryDictionaryPhase1() {
    ...
    for (int j = 1; j <= this.nbNonBasicVar; j++)
      dic.arrayNonBasicVar[j] = this.arrayNonBasicVar[j];
    for (int i = 1; i <= this.nbBasicVar; i++)
      dic.arrayBasicVar[i] = this.arrayBasicVar[i];

    // Init of constants
    for (int i = 1; i <= this.nbBasicVar; i++)
      dic.D[i][0] = this.D[i][0];

    // Init of other coefficients of D
    for (int i = 1; i <= this.nbBasicVar; i++) {
      for (int j = 1; j <= this.nbNonBasicVar; j++)
        dic.D[i][j] = this.D[i][j];
    }

    for (int i = 1; i <= nbBasicVar; i++) {
      if (dic.D[i][0] < 0)
        dic.D[i][dic.nbNonBasicVar] = 1;
    }
   ...
   * of the first phase (the dictionary obtained by the method
   * firstDicoPbAuxiliary);
   * modifies this dictionary to have a first feasible dictionary for the first
   * phase.
   */
  ...
  /**
   * From a dictionary of the first phase (dictionary of the auxiliary problem)
   * such as the value of the
   * objective function in the associated basic solution is 0 (which shows that
   * the initial problem is feasible),
   * computes the first dictionary of the second phase. Do not change the
   * dictionary. For it :
   * <br>
   * - we remove the variable x0 (the number columns decreases by 1);
   * <br>
   * - we compute the objective function according to the non-basis variables
   *
   * @param zInitial  the linear part of the objective function expressed
   *                  according to the decision variables
   * @param z0Initial the constant of the objective function expressed according
   *                  to the decision variables
   * @ param initialNonBasicVar the non-basic variables before starting the first
   * phase
   * @return returns the resulting dictionary giving the initial dictionary of the
   *         second phase
   */
  public Dictionary initialDictionaryPhase2(double[] zInitial, double z0Initial, int[] initialNonBasicVar) {
    ...
    if (columnX0 == 0)
      return null;
    // Init of basic and non-basic variables
    for (int j = 1; j < columnX0; j++)
      dict.arrayNonBasicVar[j] = arrayNonBasicVar[j];
    for (int j = columnX0; j < nbNonBasicVar; j++)
      dict.arrayNonBasicVar[j] = arrayNonBasicVar[j + 1];

    for (int i = 1; i <= nbBasicVar; i++)
      dict.arrayBasicVar[i] = arrayBasicVar[i];
    ...
    /*
     * zInitial [j] is the coefficient in z of the variable of number j + 1 in the
     * initial dictionary
   ...
   * @return if the variable indicated in parameter is in base, returns the index
   *         of
   ...
   *
   * @param var the number of the searched variable
   * @return if the variable specified in parameter is non-basic, returns the
   *         index
   *         of this variable (between 1 and nbColumns);
   *         otherwise, returns 0
   */
  public int nonBasicIndex(int var) {
    for (int j = 1; j <= nbNonBasicVar; j++)
      if (arrayNonBasicVar[j] == var)
        return j;
    return 0;
  }
   ...
   * @return the two lists in an array of two ArrayList of indices of entering
   *         variables and
  ...
  public ArrayList<Integer>[] enteringLeaving(ArrayList<Integer> listBasic) {
    ...
    for (Integer numE : listBasic) {
      ...
      if (jE != -1) {// la variable de numero numE est hors base, à l'indice jE
        ...
        while (listBasic.contains(arrayBasicVar[iS]))
          iS++;
   ...
   * nbNonBasicVar = 2
   * nbBasicVar = 3
   * arrayNonBasicVar is of dimension 3 and contains at indices 1 an 2, the
   * integers 1, 2 (for decision variables x1 and x2)
   * arrayBasicVars of dimension 4 and contains at indices 1, 2 and 3, the
   * integers 3, 4, 5 (for the slack variables x3, x4, x5)
   * The matrix D contains:
   * 0 4 3
   * 24 -2 -3
   * 30 -5 -3
   * 18 -1 -3
   * REMARK: we will say that the variable x1 is the variable of number 1, the
   * variable x2 is the variable of number 2, ...
   * When we talk about index, it will always be indexes in arrays and not
   * variable indices.
   */
  ...
  public Dictionary(Matrix A, Matrix B, ArrayList<Integer> base, double[] b, double[] zDeb, double z0Deb) {
    ...
    for (int x : base)
      columns.add(x - 1); // column contains the numbers of the basic variables decreased by 1
  ...
  public void setMethod(EnteringMethod method) {
    this.method = method;
    if (!isWritten)
      Simplex.output.println("We go to the method: " + method + "\n");
    isWritten = !isWritten;
  }
   ...
   *
   * @param bland if the parameter is true, the Bland rule will be used, otherwise
   *              it will not be
   *
   */
  ...
  public void setBland(boolean bland) {
    ...
    if (bland)
      Simplex.output.println("We use the criterion of Bland\n");
    else
      Simplex.output.println("We no longer use the Bland criterion\n");
  }
